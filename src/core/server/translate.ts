...
export class FragmentTranslationError extends Error {
  ...
  constructor(
    ...
    public result?: Record<number, string>,
  ) {
    super(message)
  }
}
...
export async function translate(
  ...
  if ("type" in input) {
    ...
    text = trimmedLines
      .map((line) => (line.type == "br" ? "" : line.content))
      .join("\n")
  }

  const result = await service.translate(
    applyGlossaries(text, opts.glossaries),
    opts,
  )
  if ("type" in input) return result

  const splitted = result.split("\n")
  if (splitted.length !== trimmedLines!.length)
    throw new FragmentTranslationError(
      "Mismatching length between fragment lines and translation results",
      input,
      result,
    )
